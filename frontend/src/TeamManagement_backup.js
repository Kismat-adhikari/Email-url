...
const TeamManagement = () => {
    ...
    // Base API targets: try backend on 5000 when running React dev server, otherwise use same-origin
    const apiTargets = useMemo(() => {
        const targets = ['']; // same-origin default
        try {
    ...
    // Resilient fetch that falls back across targets on network errors
    const fetchTeamApi = useCallback(async (path, options = {}) => {
        let lastError = null;
        for (const base of apiTargets) {
            try {
                const res = await fetch(`${base}${path}`, options);
                // If we got a response (even 4xx/5xx), return it; callers decide
                return res;
            } catch (err) {
                lastError = err;
            }
        }
        throw lastError || new Error('All team API endpoints failed');
    }, [apiTargets]);
    ...
    // Define getAuthHeaders early so hydration effect can use it
    ...
    // Hydrate user from backend if we have a token but no user (prevents skeleton loop)
    useEffect(() => {
        let abort = false;
        ...
        hydrateUser();
        return () => { abort = true; };
    }, [authToken, fetchTeamApi, getAuthHeaders]);
    ...
    const [teamQuickInfo, setTeamQuickInfo] = useState(null); // FAST: Quick info loaded first
    const [loading, setLoading] = useState(true);
    const [loadingFullTeamInfo, setLoadingFullTeamInfo] = useState(false); // LAZY: Full load happens later
    const isFetchingStatus = useRef(false); // prevent fetch loops
    ...
    // FAST: Check user status and team eligibility (quick load)
    const checkUserStatus = useCallback(async () => {
        ...
        try {
            ...
            // Only fetch status - this is very fast
            const response = await fetchTeamApi('/api/team/status', {
                headers: getAuthHeaders()
            });
            ...
            // Update user subscription tier locally if provided
            if (data.subscription_tier && user) {
                const updatedUser = { ...user, subscriptionTier: data.subscription_tier };
                setUser(updatedUser);
                localStorage.setItem('user', JSON.stringify(updatedUser));
            }
            // Fallback: if API says no, but local user is starter/pro, allow create
            const localEligible = user && ['starter','pro'].includes((user.subscriptionTier || '').toLowerCase());
            setCanCreateTeam(data.can_create_team || (!data.in_team && localEligible));
            
            // If user is in a team, load QUICK info first (very fast)
            if (data.in_team) {
                loadTeamQuickInfo();
            } else {
                setTeamInfo(null);
            ...
            // Fallback to local user tier so Pro users can still create
            const localEligible = user && ['starter','pro'].includes((user.subscriptionTier || '').toLowerCase());
            setCanCreateTeam(localEligible);
            setError('Unable to reach team service. Using local plan info.');
        } finally {
            setLoading(false);
            isFetchingStatus.current = false;
        }
    }, [authToken, navigate, getAuthHeaders, user]);

    // LAZY: Load quick team info first (like Facebook profile - loads immediately)
    const loadTeamQuickInfo = useCallback(async () => {
        try {
            const response = await fetchTeamApi('/api/team/quick-info', {
                headers: getAuthHeaders()
            });
            
            if (response.ok) {
                const data = await response.json();
                setTeamQuickInfo(data); // Set quick info immediately
                
                // THEN load full team info in background (lazy load)
                loadTeamInfoFull();
            ...
            console.error('Error loading quick team info:', err);
            setError('Quick team info failed to load. Retrying…');
        }
    }, [getAuthHeaders]);

    // FULL: Load complete team info in the background (after quick load)
    const loadTeamInfoFull = useCallback(async () => {
        try {
            setLoadingFullTeamInfo(true);
            const response = await fetchTeamApi('/api/team/info', {
                headers: getAuthHeaders()
            });
            
            if (response.ok) {
                const data = await response.json();
                setTeamInfo(data); // Now update with full info
            }
        } catch (err) {
            console.error('Error loading full team info:', err);
            setError('Team details failed to load. Please refresh.');
        } finally {
            setLoadingFullTeamInfo(false);
        }
    }, [getAuthHeaders]);

    // Trigger status check when user is hydrated (prevents skeleton loop)
    useEffect(() => {
        if (user && authToken && loading === true) {
            checkUserStatus();
        }
    }, [user, authToken, checkUserStatus, loading]);
    ...
    useEffect(() => {
        ...
        // Immediate refresh when page becomes visible or gains focus
    ...
    const createTeam = async (e) => {
        ...
        try {
            ...
            const response = await fetchTeamApi('/api/team/create', {
                method: 'POST',
                headers: getAuthHeaders(),
                body: JSON.stringify({
                    name: teamName.trim(),
    ...
    const generateInviteLink = async () => {
        try {
            ...
            const response = await fetchTeamApi('/api/team/invite', {
                method: 'POST',
                headers: getAuthHeaders(),
                body: JSON.stringify({
                    email: genericEmail,
    ...
    const removeMember = async (memberId) => {
        ...
        try {
            setButtonLoading(true);
            const response = await fetchTeamApi(`/api/team/members/${memberId}/remove`, {
                method: 'DELETE',
                headers: getAuthHeaders()
            });
    ...
    const leaveTeam = async () => {
        ...
        try {
            setButtonLoading(true);
            const response = await fetchTeamApi('/api/team/leave', {
                method: 'POST',
                headers: getAuthHeaders()
            });
    ...
    // If we have a token but no user data yet, show loading once while hydrating
    if (!user && authToken) {
        return (
            <div className="App">
                {/* Top Navigation Bar */}
                    ...
                    <div className="navbar-container">
                        ...
                        <div className="navbar-right">
                            ...
                            <div className={`api-usage-counter ${
                                ...
                                const currentLimit = isInTeam ? (user?.teamInfo?.quota_limit || 10000000) : getCorrectApiLimit((user?.subscriptionTier) || 'free');
                                ...
                                <span className="usage-text">
                                    {(() => {
                                        ...
                                        if (isInTeam) {
                                            ...
                                            return formatApiUsageWithPeriod((user?.apiCallsCount || 0), user?.apiCallsLimit, (user?.subscriptionTier) || 'free');
                                        }
                                    })()}
                                </span>
                                <span className="usage-label">
                                    {(user?.teamId && user?.teamInfo) ? 'Team Quota' : 
                                     ...
                                     (user?.subscriptionTier || '').toLowerCase() === 'pro' ? 'Pro' : 'API Calls')}
                                </span>
                            </div>
        ...
        <div className="App">
                ...
                <div className="navbar-container">
                    ...
                    <div className="navbar-right">
                        ...
                        <div className={`api-usage-counter ${
                            ...
                            {['free', 'starter'].includes((user?.subscriptionTier || '').toLowerCase()) && (user?.apiCallsCount || 0) >= getCorrectApiLimit((user?.subscriptionTier) || 'free') && (
                                <div className="upgrade-hint">
                                    <FiZap style={{marginRight: '4px'}} /> {(user?.subscriptionTier || 'free') === 'free' ? 'Upgrade to Starter!' : 'Upgrade to Pro!'}
                                </div>
                            )}
                            {(user?.subscriptionTier || '').toLowerCase() === 'pro' && (user?.apiCallsCount || 0) >= getCorrectApiLimit((user?.subscriptionTier) || 'pro') * 0.8 && (
            ...
            {(teamQuickInfo || teamInfo) && (
                <div className="team-dashboard">
                            ...
                            <div className="team-stats">
                                <div className="stat">
                                    ...
                                    <span className="value">
                                        {teamQuickInfo?.team?.member_count || teamInfo?.members?.length || 0}
                                        {teamInfo?.team?.max_members ? `/${teamInfo.team.max_members}` : ''}
                                    </span>
                                </div>
                                <div className="stat">
                        ...
                        <div className="quota-info">
                            ...
                            <div className="quota-bar">
                                <div 
                                    ...
                                    style={{ 
                                        width: `${(() => {
                                            const quotaUsed = (teamQuickInfo?.team?.monthly_validations_used || teamInfo?.team?.quota_used) || 0;
                                            const quotaLimit = (teamQuickInfo?.team?.monthly_validations || teamInfo?.team?.quota_limit) || 10000000;
                            ...
                            <div className="quota-details">
                                <span>
                                    {((teamQuickInfo?.team?.monthly_validations_used || teamInfo?.team?.quota_used) || 0).toLocaleString()} / 
                                    {((teamQuickInfo?.team?.monthly_validations || teamInfo?.team?.quota_limit) || 10000000).toLocaleString()} validations
                                </span>
                                <span>
                                    {(() => {
                                        const quotaUsed = (teamQuickInfo?.team?.monthly_validations_used || teamInfo?.team?.quota_used) || 0;
                    ...
                    {/* Team Actions - Only show if we have full info */}
                    {teamInfo && (teamInfo.user_role === 'owner' || teamInfo.user_role === 'admin') && (
                    ...
                    {/* Team Members - Show skeleton if loading full info */}
                    <div className="team-members">
                        <h4>Team Members ({teamInfo?.members?.length || 'Loading...'})</h4>
                        {!loadingFullTeamInfo && teamInfo?.members ? (
                            ...
                            <div className="loading-skeleton" style={{ padding: '20px' }}>
                                <div style={{ background: '#f8f9fa', borderRadius: '8px', padding: '16px', marginBottom: '12px', height: '60px', animation: 'pulse 2s infinite' }}></div>
                                <div style={{ background: '#f8f9fa', borderRadius: '8px', padding: '16px', height: '60px', animation: 'pulse 2s infinite' }}></div>
                            </div>
                        )}
                    </div>

                    {/* Pending Invitations - Only show if we have full info */}
                    {teamInfo && teamInfo.pending_invitations && teamInfo.pending_invitations.filter(invite => 
                    ...
                    {/* Leave Team */}
                    {teamInfo && teamInfo.user_role !== 'owner' && (
